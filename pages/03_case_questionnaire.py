...
from utils import page_setup, get_case_index, save_widget
...
if st.button(
    "Next",
    disabled=(st.session_state['confidence_level'] is None
              or st.session_state['contentment_level'] is None)):

    save_widget(
        "confidence_level",
        f"case_{get_case_index()}_confidence_level")
    save_widget(
        "contentment_level",
        f"case_{get_case_index()}_contentment_level")
